...
{{define "SetAFK"}}
  {{$Nick := sdict}}
  {{range $I, $N := (cslice .Member.Nick .Member.User.Globalname .Member.User.Username)}}
    {{if and $N (eq $I 0)}}
      {{$Nick = sdict "Type" "Guild" "Nick" $N}}
      {{break}}
    {{else if $N}}
      {{$Nick = sdict "Type" "Global" "Nick" $N}}
      {{break}}
    {{end}}
  {{end}}

  {{$MyAFK := sdict "Message" .NoM "Until" "" "Seen" currentTime.Unix "Nick" $Nick}}
  {{$AFKMessage := .NoM}}
...
{{$MyAFK.Set "Until" (currentTime.Add (toDuration (index $AFKMessage 1))).Unix}}
  ...
  {{if not $MyAFK.Until}}
    {{dbSet (toInt .Member.User.ID) "MyAFK" $MyAFK}}
  {{else}}
    {{dbSetExpire (toInt .Member.User.ID) "MyAFK" $MyAFK (sub $MyAFK.Until currentTime.Unix)}}
  {{end}}
...
{{if not $AFK}}
  {{if .Args}}
    {{if eq (lower (index .Args 0)) (lower $Command)}}
      {{$MyAFK := execTemplate "SetAFK" (sdict "Member" .Member "CmdArgs" .CmdArgs "NoM" $NoM "Separator" $Separator)}}
      ...
      {{if $NickAFK}}
        {{try}}
          {{editNickname (print $NickAFK $MyAFK.Nick.Nick)}}
        {{catch}}
        {{end}}
      {{end}}
    {{end}}
...
{{else}}
  ...
  {{if $Removed}}
    ...
    {{if $NickAFK}}
      {{try}}
        {{if eq $AFK.Value.Nick.Type "Guild"}}
          {{editNickname (print $AFK.Value.Nick.Nick)}}
        {{else}}
          {{editNickname ""}}
        {{end}}
      {{catch}}
...
{{$InAFK := cslice}}
{{range $Index, $User := $UsersInQuestion}}
...
{{range $Index, $User := $InAFK}}  
  ...
  {{end}}
  {{if (reFind `\n` .Value.Message)}}
    {{$AFKText = print "-# <@" $User.UserID ">'s been AFK for " (execTemplate "Until" (toInt64 (add (sub currentTime.Unix .Value.Seen) currentTime.Unix))) " till " $Until ":\n" (trimSpace .Value.Message) "\n" $Separator}}
  {{else}}
    {{$Separator = print "    " $Separator}}
    {{$AFKText = print "> -# <@" $User.UserID ">'s been AFK for " (execTemplate "Until" (toInt64 (add (sub currentTime.Unix .Value.Seen) currentTime.Unix))) " till " $Until ":\n " (trimSpace .Value.Message) "\n" $Separator}}
  {{end}}
  {{if and (eq $Index 0) (ge (len $AFKText) 1950)}}
    {{$Output = print $Output (trimSpace (slice $AFKText 0 1949)) "...ᴹᵒʳᵉ"}}
  {{else if lt (add (len $Output) (len $AFKText)) 1985}}
    {{$Output = print $Output $AFKText}}
  {{else}}
    {{$Output = print $Output " ～ +0" (sub (len $InAFK) $Index) " AFK"}}
    {{break}}
  {{end}}
{{end}}
